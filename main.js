    ...
    if (message.text) {
      if (message.text == "/start") {
        await bot.sendMessage(
          chatId,
          `Salom ${name}. Menga kalit so'z yuboring va men shu sozga mos rasm yuboraman.\n(Kamida 5 ta harfdan iborat so'z!) \n\nHello ${name}. Send me a keynote and I'll send you a picture to match.\n(The word should consist minimum 5 characters!)`
        );
      } else {
        if (message.text.length < 5) {
          ...
          async function secrch() {
            const API_KEY =
              "563492ad6f917000010000011d289017e2cf4800845984d427bd58a6";
            const data = await fetch(
              `https://api.pexels.com/v1/search?query=${message.text}`,
              {
                method: "GET",
                headers: {
            ...
            for (let len = 1; len < length; len++) {
              ...
              await bot.sendPhoto(chatId, image_url, {
                parse_mode: "HTML",
                caption: `<b>Photographer is</b> ${photographer}. \n\n Saved by @photoFinderBx2bot`,
              });
            }
          }
          secrch();
      ...
      await bot.sendMessage(chatId, "<b>Please send text!</b>", {
        parse_mode: "HTML",
      });
    }
  } catch (error) {
